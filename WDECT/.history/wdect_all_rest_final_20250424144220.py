...
from sklearn.datasets import load_wine, load_breast_cancer, fetch_openml, make_moons, make_circles
...
def load_and_prepare_dataset(dataset_name: str):
    ...
    if dataset_name == "wine":
        ...
        data = fetch_openml(name='house-votes-84', version=1, parser='auto')
        ...
        labels = torch.tensor(data.target.cat.codes, dtype=torch.long)
        dim = features.shape[1]  # 16
        num_classes = len(set(data.target.cat.codes))  # 2
