...
class WECTClassifier(nn.Module):
    def __init__(self, config: ECTConfig, hidden_dim: int, num_classes: int, num_directions: int):
        ...
        # build unit-circle directions tensor v of shape [2, num_directions]
        angles = torch.linspace(0, 2*math.pi, steps=num_directions, endpoint=False)
        v = torch.stack([torch.cos(angles), torch.sin(angles)], dim=0)

        # WECTLayer will compute the weighted, differentiable ECT
        self.wect_layer = WECTLayer(config, v)

        # FC layers: input dim = num_directions * bump_steps
        in_dim = num_directions * config.bump_steps
        self.fc1 = nn.Linear(in_dim, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, num_classes)

    def forward(self, batch: Batch):
        # compute WECT → [batch_size, num_directions, bump_steps]
        wect_out = self.wect_layer(batch)

        # flatten for MLP
        feats = wect_out.view(batch.num_graphs, -1)
        x = F.relu(self.fc1(feats))
        return self.fc2(x), feats  # return feats for regularization
